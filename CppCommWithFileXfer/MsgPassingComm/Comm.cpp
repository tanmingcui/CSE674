...
bool Sender::sendFile(Message msg)
{
  ...
  if (msg.containsKey("verbose")){
	  std::string sizeinfo = std::to_string(size) + " " + "bytes.";
	  EndPoint en = msg.to();
	  std::cout << "\n Sent file \"" << FileSystem::Path::getName(msg.value("sendingFile")) << "\" to " << en.toString() << ". Size " << sizeinfo << " Total sent blocks: " << totalpack;
  }
  sendFile.close();
  return true;
}
...
public:
  ...
  */
  ...
  {
	  std::string fileName = FileSystem::Path::getName(msg.value("sendingFile"));
	  std::string fileSpec = FileSystem::Path::fileSpec(msg.value("destination"), fileName);
    ...
    while (true){
      ...
      if (msgString.length() == 0)break;
      msg = Message::fromString(msgString);
	  if (msg.contentLength() == 0)   break;
    }
	if (msg.containsKey("verbose")){
		std::string sizeinfo = std::to_string(size) + " " + "bytes.";
		EndPoint en = msg.from();
		std::cout << "\n Received file \"" << FileSystem::Path::getName(msg.value("sendingFile")) << "\" from " << en.toString() << ". Size " << sizeinfo << " Total received blocks: " << totalpack;
	}
    saveStream.flush();
    saveStream.close();
